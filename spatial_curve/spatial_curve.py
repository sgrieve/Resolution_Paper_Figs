...
def Process(files):
    ...
    for i in range(1, 7):
        ...
        y_min = 0
        print np.min(hillshade)
        print np.max(hillshade)
        # plot the hillshade on the axes
        plt.imshow(hillshade, vmin=-0.5, vmax=0.5, cmap=cmx.seismic)
    ...
    # colorbar placement = [left, bottom, width, height]
    cbaxes = fig.add_axes([0.068, 0.05, 0.276, 0.01])
    ...
    cb.set_label('Curvature ($m^{-1}$)', size=8, labelpad=-26)
    cb.ax.tick_params(labelsize=8, bottom='off', pad=0)
...
def Make_The_Figure():
    ...
    Curv_files = [path + f for f in Curv_files]
